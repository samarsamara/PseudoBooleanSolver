...
class Solver {
	...
	vector<std::vector<std::pair<int, int>>> pbConstraints;  // Store PB constraints
	unordered_map<int, int> rhs_values;
	unordered_map<int, int> lhs_values;
	...
	void set_state(int x, VarState y) { state[x] = y; }
	VarState get_state_pb(int x) { return state_pb[x]; }
	void set_state_pb(int x, VarState y) { state_pb[x] = y; }
	void set_lhs_constraint(int lhs, int constraint_index) { lhs_values[constraint_index] = lhs; }
...
public:
	...
	void read_pb(std::ifstream& in);

	// Function to convert pseudo-Boolean constraints to CNF using additive networks
	std::vector<Clause> convert_pb_to_cnf(const std::vector<std::pair<int, int>>& terms, int rhs);



	void normalizePBConstraint(std::vector<std::pair<int, int>>& pb_constraint, int& num_constraint, int& rhs, bool bigger);
	int choose_best_variable();
	void propagatePBConstraints(int assigned_var, VarState state , bool flip);
	void backtrack_pb();
	void applySATSolution();

	std::vector<int> findConflictSubset(const std::vector<std::pair<int, int>>& constraint);
	void handleConflict(const std::vector<std::pair<int, int>>& constraint);

	// Helper function for full adder circuit in the CNF conversion
	std::vector<Clause> add_full_adder_clauses(int a, int b, int c_in, int sum, int c_out);
	// Helper function for half adder circuit in the CNF conversion
	std::vector<Clause> add_half_adder_clauses(int a, int b, int sum, int c_out);
	
	void print_variable_count(int var);

	bool solvePBConstraints();
    // debugging
	// Print the buckets
	void print_buckets(const std::vector<std::vector<int>>& buckets) {
		...
		for (size_t i = 0; i < buckets.size(); ++i) {
			std::cout << "Bucket " << i << ": ";
			for (int var : buckets[i]) {
				std::cout << var << " "; // Print each variable in the bucket
			}
			std::cout << std::endl;
		}
	}
...
private:
	int variable_count = 0; // Tracks the total variables for PB handling
};
